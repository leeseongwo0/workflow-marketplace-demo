...
| --- | --- | --- |
| `/` | `/marketplace`로 이동 | — |
| `/marketplace` | 마켓플레이스 목록 | 구현 |
| `/marketplace/:workflowId` | 워크플로 상세 | 구현 |
| `/search` | 검색 | 구현 |
| `/profile` | 내 자산 (구매/등록한 워크플로) | 구현 |
| `/register` | 워크플로 등록 | placeholder |
| `/execute/:id` | 워크플로 실행 | placeholder |

목록·상세·프로필은 아직 화면용 목업 데이터를 사용합니다. 실제 온체인 데이터와
연결하는 작업은 `google_news_rss/v1` 워크플로부터 진행 중입니다.

이전 `/app` 경로(`LiveDemoPage`)는 위 화면들로 대체되어 더 이상 라우팅되지
않습니다. 해당 파일은 온체인 구매·실행 흐름이 실제로 동작하는 참조 구현이라
이관이 끝날 때까지 남겨 둡니다.
