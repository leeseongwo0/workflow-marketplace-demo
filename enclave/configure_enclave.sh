...
# aiwf-executor: this Rust binary only serves /get_attestation on port 3000.
# The real API is the existing Node executor (apps/executor in
# workflow-marketplace-demo), started as a sibling process inside the
# enclave and listening on EXECUTOR_PORT (3001 per the team's .env). Forward
# that port too so the Node app's full API is reachable from outside.
if [ "$ENCLAVE_APP" = "aiwf-executor" ]; then
    echo "Adding aiwf-executor port 3001 vsock listener to run.sh..."
    if [[ "$(uname)" == "Darwin" ]]; then
        sed -i '' '/socat VSOCK-LISTEN:3000,reuseaddr,fork TCP:localhost:3000 &/a\
\
# For aiwf-executor: forward to the Node executor process on port 3001\
socat VSOCK-LISTEN:3001,reuseaddr,fork TCP:localhost:3001 &' src/nautilus-server/run.sh
    else
        sed -i '/socat VSOCK-LISTEN:3000,reuseaddr,fork TCP:localhost:3000 &/a\
\
...
socat VSOCK-LISTEN:3001,reuseaddr,fork TCP:localhost:3001 &' src/nautilus-server/run.sh
    ...
    echo "Added port 3001 vsock listener for aiwf-executor"

    echo "Adding Node executor launch to run.sh..."
    if [[ "$(uname)" == "Darwin" ]]; then
